...
func TestKimiDeviceFlowPersistsWithoutLeakingSecrets(t *testing.T) {
	access := jwt(map[string]any{"user_id": "kimi-1", "email": "Ada@Kimi.com", "exp": time.Now().Add(time.Hour).Unix()})
	refresh := jwt(map[string]any{"user_id": "kimi-1"})
	...
	home := t.TempDir()
	pendingLogin, err := NewPendingLogin(context.Background(), home)
	if err != nil {
		t.Fatal(err)
	}
	if pendingLogin.UserCode != "ABCD" || strings.Contains(pendingLogin.AuthURL(), "dev-secret") {
	...
	done, err := file.Complete(context.Background(), home)
	if err != nil {
		t.Fatal(err)
	}
	if done.Account.ID != "kimi-1" || done.Account.Email != "ada@kimi.com" {
	...
	if !strings.Contains(string(raw), `"kimi"`) || strings.Contains(string(raw), "dev-secret") == false && !strings.Contains(string(raw), access) {
		t.Fatalf("%s", raw)
	}
	accounts, active, err := antigravity.LoadPublicAccountsFor(auth, ProviderID)
	if err != nil || active != "kimi-1" || len(accounts) != 1 {
