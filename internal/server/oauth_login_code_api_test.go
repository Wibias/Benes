...
func TestOAuthLoginCodeFailsClosedWithoutPending(t *testing.T) {
	...
	blocked := httptest.NewRequest(http.MethodPost, "/api/oauth/login/code", strings.NewReader(`{"provider":"google-antigravity","input":"code"}`))
	...
	req := httptest.NewRequest(http.MethodPost, "/api/oauth/login/code", strings.NewReader(`{"provider":"google-antigravity","input":"code"}`))
	req.Host = "127.0.0.1"
