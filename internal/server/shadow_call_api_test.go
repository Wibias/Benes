...
func TestShadowCallAPIPersistsWithoutSecrets(t *testing.T) {
	...
	blocked := httptest.NewRequest(http.MethodGet, "/api/shadow-call-settings", nil)
	...
	put := httptest.NewRequest(http.MethodPut, "/api/shadow-call-settings", strings.NewReader(`{"enabled":true,"model":"gpt-5.5"}`))
	put.Host = "127.0.0.1"
	putRR := httptest.NewRecorder()
	h.ServeHTTP(putRR, put)
	if putRR.Code != http.StatusOK || strings.Contains(putRR.Body.String(), "sk-secret") || !strings.Contains(putRR.Body.String(), `"enabled":true`) {
