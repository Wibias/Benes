param(
  ...
  [Parameter(Mandatory = $true)][string]$BenesHome,
...
$BenesHome = Normalize-HomePath $BenesHome
...
$onlineIcon = Load-TrayIcon "benes-tray-online.ico" ([System.Drawing.SystemIcons]::Information)
$warningIcon = Load-TrayIcon "benes-tray-warning.ico" ([System.Drawing.SystemIcons]::Warning)
$offlineIcon = Load-TrayIcon "benes-tray-offline.ico" ([System.Drawing.SystemIcons]::Error)
...
$stableHash = Get-StableHash $BenesHome
$stopEventCreated = $false
$stopEvent = New-Object System.Threading.EventWaitHandle($false, [System.Threading.EventResetMode]::AutoReset, "Local\BenesTrayStop-$stableHash", [ref]$stopEventCreated)
if ($Mode -eq "Stop") {
  [void]$stopEvent.Set()
  $stopEvent.Dispose()
  exit 0
...
$createdNew = $false
$mutex = New-Object System.Threading.Mutex($true, "Local\BenesTray-$stableHash", [ref]$createdNew)
if (-not $createdNew) {
  $stopEvent.Dispose()
  $mutex.Dispose()
  exit 0
...
$heartbeatPath = Join-Path $BenesHome "tray-heartbeat.json"
$actionLogPath = Join-Path $BenesHome "tray-actions.log"
...
function Start-BenesCommand([string[]]$CommandArgs, [switch]$TrackExit) {
  try {
    $psi = New-Object System.Diagnostics.ProcessStartInfo
    $psi.FileName = $CliPath
    $psi.Arguments = (($CommandArgs | ForEach-Object { ConvertTo-NativeArgument $_ }) -join " ")
    $psi.UseShellExecute = $false
    $psi.CreateNoWindow = $true
    $psi.WindowStyle = [System.Diagnostics.ProcessWindowStyle]::Hidden
    $psi.EnvironmentVariables["CODEX_HOME"] = $CodexHome
    $psi.EnvironmentVariables["BENES_HOME"] = $BenesHome
    ...
    $notify.ShowBalloonTip(5000, "benes action failed", "The action could not start. Open the logs folder or run benes doctor.", [System.Windows.Forms.ToolTipIcon]::Error)
    return $false
  }
}

function Read-ListenTarget {
  foreach ($path in @((Join-Path $BenesHome "runtime-port.json"), (Join-Path $BenesHome "config.json"))) {
    try {
  ...
  return @{ port = 23100; host = "127.0.0.1"; pid = $null }
}
...
function Complete-PendingAction([bool]$Success) {
  ...
  if ($Success) {
    ...
    $notify.ShowBalloonTip(2500, "benes", "$action completed.", [System.Windows.Forms.ToolTipIcon]::Info)
  } else {
    Write-ActionLog "$action failed to reach the expected state"
    $notify.ShowBalloonTip(5000, "benes action failed", "$action did not reach the expected state. Open the logs folder or run benes doctor.", [System.Windows.Forms.ToolTipIcon]::Error)
  }
}
...
function Update-TrayState {
  ...
  $script:online = $null -ne $health -and $health.status -eq "ok" -and $health.service -eq "benes" -and [int]$health.port -eq $script:port -and $pidMatches
  $script:proxyPid = if ($script:online) { [int]$health.pid } else { $null }
  if ($script:online) {
    $statusItem.Text = "Proxy: Online (port $($script:port))"
    $notify.Text = "benes: Online"
...
$openItem.add_Click({ Start-BenesCommand @("gui") })
$startItem.add_Click({
  ...
  $startProcess = Start-BenesCommand @("__tray-start") -TrackExit
...
$stopItem.add_Click({
  ...
  $stopProcess = Start-BenesCommand @("stop") -TrackExit
...
$restartItem.add_Click({
  ...
  $restartProcess = Start-BenesCommand @("__tray-restart") -TrackExit
